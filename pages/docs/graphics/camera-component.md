...
To enable this mode, the `UsageHint` has to be set to `Render to Texture`. You also need to select a `CameraRenderPipeline`. The render pipeline defines how the scene is rendered and which rendering effects are applied. You need to configure which [render pipelines](render-pipeline/render-pipeline-overview.md) are available to the cameras in the [asset profiles](../assets/asset-profiles.md).
